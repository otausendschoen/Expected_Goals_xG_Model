...
We first filtered out penalty shots and ensured all shot coordinates stayed within the pitch boundaries. Then, we binned shot locations into 10x10 zones and extracted the first year from each `season_name` to assign a corresponding decade. We kept only shots from selected decades (1970s to 2020s). Finally, we grouped the data by bin and decade, counted the number of shots per bin, and calculated the proportion of shots (`shot_prop`) within each decade to allow normalized comparisons in the heatmap.
